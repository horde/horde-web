...
class Jonah_FeedParser {
    ...
    function init()
    {
        ...
        xml_set_processing_instruction_handler($this->parser, 'defaultHandler');
        xml_set_external_entity_ref_handler($this->parser, 'defaultHandler');
    }
    ...
    function startElement($parser, $name, $attribs)
    {
        ...
        case 'CHANNELURL':
        case 'STORYTEXT':
            $this->child = strtolower($name);
            if ($name == 'LINK' && $this->format == 'atom' && !empty($attribs['href'])) {
                if ($this->parent == 'FEED') {
                    if (isset($this->channel[$this->child])) {
                        $this->channel[$this->child] = '';
                    }
                    $this->channel[$this->child] .= $attribs['href'];
                } else {
                    if (!isset($this->item[$this->child])) {
                        $this->item[$this->child] = '';
                    }
                    $this->item[$this->child] .= $attribs['href'];
                }
            }
            break;
        ...
        case 'CONTENT':
            if (isset($attribs['type'])) {
                $this->item['content_type'] = $attribs['type'];
            }
            $this->child = 'body';
            break;
            ...
            if ($this->format == 'atom' && in_array($this->child, array('body', 'description'))) {
                ...
                default:
                    ...
                    foreach ($attribs as $aname => $avalue) {
                        $this->item[$this->child] .= ' ' . strtolower($aname) . '="' . $avalue . '"';
                    }
                    $this->item[$this->child] .= '>';
                    break;
                }
